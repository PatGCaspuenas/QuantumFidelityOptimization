...
function main(; seed=1, σ=0.01)
    ...
    f(x) = CalibrationCode.Q_noisy(t, init_param[2], x[1], x[2]; N=100, phase_grid=0:0.2:π, dt=0.1)
    

    bounds = [(init_param[3]*.8, init_param[3]*1.2), (init_param[4]*.8, init_param[4]*1.2)]
    ...
    println("ideal values: f_sb = ", init_param[3], ", A = ", init_param[4])

    p = plot2d_standard(res; nx=50, ny=50, obs_noise=σ)
    savefig(p, "figures/calib_standard_2d.png")
    println("Saved -> calib_standard_2d.png")

    anim, fps = animate2d_standard(res; nx=50, ny=50, fps=6, obs_noise=σ)
    gif(anim, "figures/calib_standard_2d.gif", fps=fps)
    println("Saved -> calib_standard_2d.gif")
end
