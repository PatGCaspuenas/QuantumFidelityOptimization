...
function main()
    ...
    span_hz  = 1e4  # ± span around baseline
    bounds_u = [(-1.0, 1.0), (-1.0, 1.0)]
    ...
    # -------------------------
    res_det = CalibrationCode.bayesopt(Qdet_u;
        bounds=bounds_u,
        n_init=20,
        n_iter=60,
        M=4000,
    ...
    best_idx_det = argmax(res_det.y)
    u_rec_det = vec(res_det.X[:, best_idx_det])
    y_rec_det_gpmean = res_det.y[best_idx_det]
    ...
    # -------------------------
    # 3) Visual comparison on deterministic landscape
    # -------------------------
    nx, ny = 41, 41
    ...
    savefig(p, "figures/compare_recommendations.png")
    println("\nSaved: figures/compare_recommendations.png")
end
