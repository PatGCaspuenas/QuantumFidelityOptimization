...
# trace dirs (data/traces_freqspan10_bound050_full_l1_N*_nostop100_stream_100seeds)
...
if [ "$#" -gt 0 ]; then
  ...
  tasks=(
    # Panel A: scale sweep (NInf, full_l1, bound 0.1/0.5/1.0)
    "scale_sweep:0.1"
    "scale_sweep:0.5"
    "scale_sweep:1.0"
    # Panel B: selected-N full_l1 (bound 0.5)=
    "full_l1:100"
    "full_l1:1000"
    "full_l1:10000"
    "full_l1:100000"
...
tag_for() {
  local score_mode="$1"
  local n="$2"
  if [ "$score_mode" = "scale_sweep" ]; then
    local bound_tag; bound_tag="$(scale_to_bound_tag "$n")"
    echo "freqspan10_bound${bound_tag}_NInf_lhs12_restart_fullbudget100_100seeds"
  elif [ "$score_mode" = "odd_penalty" ]; then
    echo "freqspan10_bound050_N${n}_nostop100_stream_100seeds"
  else
    echo "freqspan10_bound050_${score_mode}_N${n}_nostop100_stream_100seeds"
  fi
}
...
for task in "${tasks[@]}"; do
  ...
  if [ "$score_mode" = "scale_sweep" ]; then
    # Panel A: NInf, full_l1, variable bound scale
    n_iter="$(scale_to_iter "$n")"
    TRACE_N_WORKERS=19 \
    TRACE_N_SHOTS=Inf \
    TRACE_NUM_SIMS=100 \
    TRACE_N_INIT=12 \
    ...
    # Panel B: full_l1 / odd_penalty selected-N runs (bound 0.5)
    TRACE_N_WORKERS=19 \
    TRACE_N_SHOTS="$n" \
    TRACE_NUM_SIMS=100 \
    TRACE_N_INIT=12 \
