...
if nprocs() == 1
    addprocs(8)  # M2 has 4 performance + 4 efficiency cores
end
...
results_grid = pmap(param_pairs; batch_size=1) do (α, κ)
    ...
    try
        ...
        sim_results = map(1:num_sims) do sim_idx
            seed = seeds[sim_idx]
            println("  Sim $sim_idx / $num_sims (seed=$seed)")
            flush(stdout)
...
open(output_file, "w") do io
    ...
    println(io, "α\tκ\tavg_iters\tstd_iters\tmedian_iters\tmin_iters\tmax_iters\tavg_Q_det\tstd_Q_det")
    for r in sorted_results
        Q_dets = [s.Q_det for s in r.sims]
        println(io, "$(r.α)\t$(r.κ)\t$(r.avg_iterations)\t$(r.std_iterations)\t$(r.med_iterations)\t$(r.min_iterations)\t$(r.max_iterations)\t$(mean(Q_dets))\t$(std(Q_dets))")
    end
end
