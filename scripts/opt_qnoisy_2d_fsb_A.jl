...
include(joinpath(@__DIR__, "plots_hetero.jl"))
...
function main()
    ...
    # u = [u_fsb, u_A], f_cl fixed at baseline
    u_to_params(u) = (f_cl0,
                      f_sb0 + span_fsb*u[1],
                      A0    + span_A  *u[2])
    ...
    savefig(p, "figures/opt_qnoisy_hetero_2d_fsb_A.png")
    println("Saved -> figures/opt_qnoisy_hetero_2d_fsb_A.png")

    anim, fps = animate2d_hetero(resH; f_true=Q_true, nx=25, ny=25, fps=6, show_noise=true, learn_hypers=false)
    gif(anim, "figures/opt_qnoisy_hetero_2d_fsb_A.gif", fps=fps)
    println("Saved -> figures/opt_qnoisy_hetero_2d_fsb_A.gif")
end
