...
def plot_versions(n_pairs, version_labels=('Version 1', 'Version 2', 'Version 3'), markers=('o', '^', 's'), output_file=None, max_iterations=120):
    """
    n_pairs: list of dicts with keys:
        N     - shot count label (int)
        files - list of file paths (list of pathlib.Path or strings) corresponding to the versions
        var   - if True, treat as a variable-N pair
    version_labels: Labels for the versions plotted
    markers: Marker styles mapped to each version in the sequence
    """
    ...
    all_x, all_y = [], []
    x_range_per_N = {}  # N -> [xmin, xmax] across all versions
    ...
    for pair in n_pairs:
        ...
        for idx, filepath in enumerate(files):
            ...
            mfc    = col  # You can apply hollow logic here if preferred
            zbase  = 2
            msize  = 7
            ew     = 1.5
            ...
            # Median and Interquartile Range (IQR) summary marker
            mx = np.median(log_shots)
            my = np.median(infidelity)
            
            if len(log_shots) > 1:
                x_25, x_75 = np.percentile(log_shots, [25, 75])
                y_25, y_75 = np.percentile(infidelity, [25, 75])
            else:
                x_25, x_75 = mx, mx
                y_25, y_75 = my, my

            # Calculate absolute distances for the error bars
    ...
    ax.axhline(1-0.999, color='dimgray', linestyle='--', linewidth=2.0, alpha=0.9)

    ax.set_yscale('log')
    ax.set_xlabel('$\\log_{10}$(Total shots)', fontsize=12)
    ax.set_ylabel('$1 - Q(\mathbf{x}^*)$', fontsize=12)
    ax.tick_params(axis='both', direction='in', length=5,
                   top=False, bottom=True, left=True, right=False, labelsize=12)
    ax.grid(True, which='both', ls=':', alpha=0.15, color='black')
    ax.set_title(r'$Q_{threshold}=0.999$', fontsize=12)

    # Version legend dynamically created from versions provided
    version_handles = []
    for i, label in enumerate(version_labels):
        version_handles.append(
            mlines.Line2D([], [], color='dimgray', marker=markers[i % len(markers)], 
                         linestyle='None', markersize=7, label=label)
       )
    version_handles.append(
        mlines.Line2D([], [], color='dimgray', linestyle='--', linewidth=2.0, label='Threshold')
    )
    ...
    if any(p.get('var', False) for p in n_pairs):
        n_handles.append(mpatches.Patch(color='black', label='Var'))
    
    fig.legend(handles=n_handles, loc='outside lower center', ncol=len(n_handles),
...
if __name__ == '__main__':
    ...
    # Add as many files inside the 'files' list as you have cases. 
