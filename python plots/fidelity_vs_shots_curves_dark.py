...
def plot_versions(n_pairs, output_file=None, max_iterations=120):
    ...
    n_pairs: list of dicts with keys:
        ...
        var      - if True, treat as a variable-N pair: drawn in white with
                   diamond markers (old=hollow 'D', new=filled 'D')
    Color encodes N; marker encodes version (circle=old, triangle=new).
    var pairs are always white with diamond markers and sit on top (zorder=10).
    ...
    for pair in n_pairs:
        ...
        col    = 'white' if is_var else color_map[pair['N']]
    ...
    if is_var:
        n_handles.append(mpatches.Patch(color='white', label='Var'))
